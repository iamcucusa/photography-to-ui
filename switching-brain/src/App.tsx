...
export default function App() {
  ...
  // Plain node ids are the single source of truth for "which node is active",
  // shared between the hero graph and (soon) the network lanes. hover/pinned above
  // still carry cx/cy for the anchored InspectCard placement (graph-originated only).
  ...
  const bias = -axis
  const current = pinned ?? hover // drives the anchored card (needs cx/cy)
  const inspectedId = selectedNodeId ?? hoveredNodeId // drives graph + lane highlight

  const canvasRef = useRef<HTMLDivElement | null>(null)
  const outputGlow = useRef<SVGCircleElement | null>(null)
  // Ref-backed activation bus: written each frame in handleFrame, read by the
  // lanes in their own rAF. Stable object identity so the context value never churns.
  const activationRef = useRef<Activation | null>(null)
  const activationBus = useMemo(() => ({ latest: activationRef }), [])
  ...
  const handleFrame = useCallback((a: Activation) => {
    // Publish the frame's activation for the lanes to read (ref, not state).
    activationRef.current = a
  ...
  const onInspectHover = useCallback(
    (t: InspectTarget | null) => {
      ...
      setHoveredNodeId(t?.node.id ?? null)
    },
    [poke],
  )
  ...
  const onInspectActivate = useCallback(
    (t: InspectTarget) => {
      ...
      setSelectedNodeId((id) => (id === t.node.id ? null : t.node.id))
    },
    [poke],
  )
  ...
  return (
    <ActivationContext.Provider value={activationBus}>
      <main className="app">
